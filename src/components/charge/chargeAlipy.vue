<template>
  <div class="charge">
    ...
    <div class="content">
      <div class="back_cv">
        <div class="leuoj" style="margin-bottom: 0;">充值二维码（长按保存到手机）</div>
        <div class="giun">
          <img :src = "alipay" alt="收款码">
        </div>
      </div>
      <div class="back_cv">
        <div class="leuoj">充值信息</div>
        <div class="pasfm" style="margin-bottom: 0px;">
          <div class="pasfm1">&nbsp;&nbsp;&nbsp;&nbsp;充值&nbsp;&nbsp;&nbsp;</div>
          <input type="number" placeholder="请输入充值数量" class="ipt" v-model="num" @blur="mpokl($event)">
          <span>X {{ unit_pay.CNY }}</span>
        </div>
        <div class="xikol">共充值 {{ num * unit_pay.CNY }} CNY = {{ num * unit_pay.USDT }} USDT</div>
        <div class="pasfm">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$post('/coin/alipay/get', {unit: this.unit})
        .then(res => {
          this.alipay = res.data.alipay_img
          this.unit_pay = res.data.exchangerate
        })
    },
    qqw () {
      this.addimg()
    ...
    submit () {
      if (this.num) {
        if (this.address) {
          if (this.trade_pwd) {
            if (this.img_id) {
              this.$post('/coin/recharge/alipay', {num: this.num, address: this.address, trade_pwd: this.trade_pwd, unit: this.unit, recharge_card_id: this.img_id})
                .then(res => {
                  if (res.status === 0) {
                    this.$message({
                      message: res.msg,
